...
async def create_resource(owner_id, resource_type, title, username, chat_id, category, description, social_link=""):
    async with aiosqlite.connect(DATABASE_PATH) as db:
        c = await db.execute("INSERT INTO resources (owner_id,resource_type,title,username,chat_id,category,description,social_link) VALUES (?,?,?,?,?,?,?,?)", (owner_id,resource_type,title,username,chat_id,category,description,social_link))
        await db.commit()
        return c.lastrowid
