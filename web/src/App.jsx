                    ...
                    return (<div className="fixed inset-0 bg-black/70 z-[60] flex items-end sm:items-center justify-center" onClick={onClose}>
                        <div className="glass-strong rounded-t-3xl sm:rounded-3xl p-6 w-full sm:max-w-md max-h-[85vh] overflow-y-auto space-y-4 page-enter" onClick={e => e.stopPropagation()}>
                            {loading ? <SkeletonList count={2} /> : !data ? <p className="text-white/40">Не найдено</p> : <>
                                <div className="flex items-center gap-3"><span className="text-3xl">{data.resource_type === 'channel' ? '📣' : data.resource_type === 'bot' ? '🤖' : '👥'}</span><div><h3 className="text-xl font-bold text-white">{data.title}</h3><p className="text-sm text-white/40">@{data.username}</p></div></div>
                                <div className="flex gap-4 text-sm"><span className="text-white/60">👥 {data.members_count ?? '—'} подписчиков</span><span className="text-yellow-400 font-bold">⭐ {(data.public_rating || 0).toFixed(1)}</span></div>
